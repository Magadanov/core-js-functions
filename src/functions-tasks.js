...
function getPolynom(...args) {
  ...
  return (x) => {
    return args.reduce((acc, cur, index) => {
      const rightId = args.length - index - 1;
      return acc + cur * x ** rightId;
    }, 0);
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    function getArgs() {
      const result = args.map((arg) => JSON.stringify(arg));
      return result.reduce((prev, curr) => `${prev},${curr}`);
    }
    logFunc(`${func.name}(${getArgs()}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${getArgs()}) ends`);
    return result;
  };
}
